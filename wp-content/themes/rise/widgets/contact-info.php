...
class rise_Contact_Info extends WP_Widget {


// constructor

    function rise_contact_info() { 

		$widget_ops = array('classname' => 'rise_contact_info_widget', 'description' => esc_html__( 'Display your contact info', 'rise') ); 

        parent::__construct(false, $name = esc_html__('MT - Contact Info Widget', 'rise'), $widget_ops); 

		$this->alt_option_name = 'rise_contact_info';
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'address' )); ?>"><?php esc_html_e( 'Enter your address', 'rise' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'phone' )); ?>"><?php esc_html_e( 'Enter your phone number', 'rise' ); ?></label>
	...
	<p><label for="<?php echo sanitize_text_field( $this->get_field_id( 'email' )); ?>"><?php esc_html_e( 'Enter your email address', 'rise' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['rise_contact_info']) )

			delete_option('rise_contact_info');		  
	...
	function flush_widget_cache() {
		...
		wp_cache_delete('rise_contact_info', 'widget');
	...
	function widget($args, $instance) {
		...
		if ( ! $this->is_preview() ) {
			...
			$cache = wp_cache_get( 'rise_contact_info', 'widget' );
		...
		$title = ( ! empty( $instance['title'] ) ) ? esc_attr( $instance['title'] ) : esc_html__( 'Contact info', 'rise' );
		...
		if( ($address) ) {
			...
			echo '<span>' . esc_html__('Address: ', 'rise') . '</span>' . wp_kses_post( $address );
		...
		if( ($phone) ) {
			...
			echo '<span>' . esc_html__('Phone: ', 'rise') . '</span>' . wp_kses_post( $phone ); 
		...
		if( ($email) ) {
			...
			echo '<span>' . esc_html__('Email: ', 'rise') . '</span>' . '<a href="mailto:' . sanitize_email( $email ) . '">' . sanitize_email( $email ) . '</a>';
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'rise_contact_info', $cache, 'widget' );  
